...
public class PlayoffServiceImpl implements PlayoffService
{
    ...
    {
        List<Player> players = requestDto.getPlayers();
        List<Team> teams = requestDto.getTeams();

        if (players.size() != 4 || teams.size() != 4)
            throw new IllegalArgumentException("Must provide exactly 4 players and 4 teams");
        ...
        playoff.setName(requestDto.getPlayoffTitle());
        ...
        // 5. Return response
        return new PlayoffResponseDto(playoff.getId(), playoff.getName(), List.of(SeriesMapper.toResponseDto(semi1),
            SeriesMapper.toResponseDto(semi2)));
    }
    ...
    @Transactional
    public PlayoffResponseDto updatePlayoffAfterSemis(Long playoffId)
    {
        ...
        List<SeriesResponseDto> responseSeries = playoff.getSeries().stream()
            .map(SeriesMapper::toResponseDto)
            .toList();

        return new PlayoffResponseDto(playoff.getId(), playoff.getName(), responseSeries);
    }
